...
contract Musicosmos is ERC721, ReentrancyGuard {
    ...
    function purchaseSong(uint256 _id) public payable nonReentrant {
        ...
        // Require that there is enough evmos provided for the transaction
        require(msg.value >= _song.price, "Not enough evmos provided");
        ...
        if (_song.currentOwnerAddress == _song.artistAddress) {
            // Transfer all evmos to artist whenever the first transaction for the NFT happens. This is because the artist is the owner after creating the NFT.
            payable(_song.currentOwnerAddress).transfer(msg.value);
            emit FundsTransferredToArtist(msg.value);
        } else {
            // Pay the seller by sending 94% evmos
            ...
            // Pay the platform fee by sending the remaining 1% evmos
